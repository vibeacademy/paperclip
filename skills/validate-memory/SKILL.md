...
description: (Memory MCP only) Check that completed tickets have CompletedTicket entities in Memory MCP. Optionally creates missing entities. Skip if Memory MCP is not configured. Largely irrelevant in Paperclip — kept for completeness.
---

# Validate Memory

> **Note for Paperclip migration:** This skill assumes a Memory MCP server is
> available. Paperclip uses task/comment threading instead of MCP-based memory,
> so this skill will short-circuit on the pre-check. Either skip importing it
> or rewrite for Paperclip's memory model.
...
FAIL Missing memory: CompletedTicket-{issue} — no entity found for #{issue} ({title})
